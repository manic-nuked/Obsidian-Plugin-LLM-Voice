...
var AIChatView = class extends import_obsidian.ItemView {
  ...
  async onOpen() {
    ...
    container.empty();
    container.createEl("h4", { text: "\u{1F916} AI Note Assistant" });
  ...
  addMessage(role, content, improveNoteAction) {
    ...
    avatar.innerHTML = role === "user" ? "\u{1F464}" : "\u{1F916}";
    ...
    if (improveNoteAction) {
      ...
      yesBtn.onclick = async () => {
        ...
        try {
          ...
          new import_obsidian.Notice(`\u2705 Updated note: ${improveNoteAction.filename}`);
          actionContainer.createEl("span", { text: " (Note updated!)", cls: "ai-chat-action-confirm" });
        } catch (err) {
          console.error("Failed to update note:", err);
          new import_obsidian.Notice(`\u274C Failed to update note: ${improveNoteAction.filename}`);
  ...
  async getNotesContext(files, query) {
    const relevantFiles = files.filter((file) => {
      const queryLower = query.toLowerCase();
      const filenameLower = file.basename.toLowerCase();
      const pathLower = file.path.toLowerCase();
      return filenameLower.includes(queryLower) || pathLower.includes(queryLower) || queryLower.includes(filenameLower);
    });
    ...
    const allRelevantFiles = [.../* @__PURE__ */ new Set([...relevantFiles, ...contentMatches])];
    const filesToCheck = allRelevantFiles.length > 0 ? allRelevantFiles : files.sort((a, b) => b.stat.mtime - a.stat.mtime).slice(0, 15);
...
`;
    ...
    for (const file of filesToCheck) {
      try {
        const content = await this.app.vault.read(file);
        const maxLength = allRelevantFiles.includes(file) ? 1e3 : 500;
...
`;
  ...
  async sendToOpenAI(message, notesContext) {
...
var AIAssistantPlugin = class extends import_obsidian.Plugin {
  constructor() {
    super(...arguments);
  ...
  async onload() {
    ...
    this.addRibbonIcon("microphone", "Voice Dictation", (evt) => {
      this.toggleVoiceRecording();
    });
    this.addRibbonIcon("message-circle", "AI Note Assistant", (evt) => {
      this.activateView();
    });
    this.addCommand({
      id: "voice-dictation",
  ...
  async startRecording() {
    try {
      ...
      new import_obsidian.Notice("\u{1F3A4} Recording started... Click again to stop");
    } catch (error) {
  ...
  stopRecording() {
    if (this.mediaRecorder && this.isRecording) {
      ...
      this.isRecording = false;
      new import_obsidian.Notice("\u{1F3A4} Recording stopped, processing...");
    }
  }
    ...
    try {
      ...
      if (data.text) {
        ...
        if (activeView) {
          ...
          new import_obsidian.Notice("\u2705 Transcription added to note");
      ...
      new import_obsidian.Notice("Error transcribing audio");
      console.error("Transcription error:", error);
    }
  }
  async aiTagCurrentNote(editor, view) {
    if (!this.settings.openaiApiKey) {
    ...
    try {
      new import_obsidian.Notice("\u{1F916} Generating AI tags...");
      const response = await fetch("https://api.openai.com/v1/chat/completions", {
        method: "POST",
        headers: {
          "Authorization": `Bearer ${this.settings.openaiApiKey}`,
...
${content}`
      ...
      if (data.choices && data.choices[0]) {
        const tags = data.choices[0].message.content.split(",").map((tag) => tag.trim().toLowerCase()).filter((tag) => tag.length > 0);
        ...
        new import_obsidian.Notice(`\u2705 Added tags: ${tags.join(", ")}`);
      }
    } catch (error) {
      new import_obsidian.Notice("Error generating tags");
      console.error("AI tagging error:", error);
    }
  }
  async extractCalendarItems(editor, view) {
    var _a;
    const content = editor.getValue();
    if (!content.trim()) {
      new import_obsidian.Notice("Note is empty");
    ...
    if (calendarItems.length > 0) {
      const calendarNote = `# Calendar Items from ${((_a = view.file) == null ? void 0 : _a.name) || "Current Note"}

${calendarItems.join("\n")}

Extracted on: ${(0, import_moment.default)().format("YYYY-MM-DD HH:mm")}`;
      await this.createOrUpdateNote("Calendar Items", calendarNote);
      new import_obsidian.Notice(`\u2705 Found ${calendarItems.length} calendar items`);
    } else {
      new import_obsidian.Notice("No calendar items found");
    }
  }
  async generateDailyTasks(editor, view) {
    var _a;
    if (!this.settings.openaiApiKey) {
      new import_obsidian.Notice("Please set your OpenAI API key in settings");
      return;
    ...
    try {
      new import_obsidian.Notice("\u{1F916} Generating daily tasks...");
      const response = await fetch("https://api.openai.com/v1/chat/completions", {
        method: "POST",
        headers: {
          "Authorization": `Bearer ${this.settings.openaiApiKey}`,
...
Generated from: [[${((_a = view.file) == null ? void 0 : _a.name) || "Current Note"}]]`;
        await this.createOrUpdateNote(`Daily Tasks ${today}`, tasksNote);
        new import_obsidian.Notice("\u2705 Daily tasks generated");
      }
    } catch (error) {
      new import_obsidian.Notice("Error generating tasks");
      console.error("Task generation error:", error);
    }
  }
  async suggestBacklinks(editor, view) {
    if (!this.settings.openaiApiKey) {
      new import_obsidian.Notice("Please set your OpenAI API key in settings");
      return;
    }
    ...
    try {
      new import_obsidian.Notice("\u{1F916} Finding backlink suggestions...");
      const allFiles = this.app.vault.getMarkdownFiles();
...
Existing notes: ${noteNames.join(", ")}`
      ...
      const data = await response.json();
      if (data.choices && data.choices[0]) {
        const suggestions = data.choices[0].message.content.split(",").map((name) => name.trim()).filter((name) => noteNames.includes(name));
        if (suggestions.length > 0) {
          const backlinks = suggestions.map((name) => `[[${name}]]`).join(" ");
          const newContent = `${content}

## Related Notes
${backlinks}`;
          editor.setValue(newContent);
          new import_obsidian.Notice(`\u2705 Added ${suggestions.length} backlink suggestions`);
        } else {
          new import_obsidian.Notice("No relevant backlinks found");
        }
      }
    } catch (error) {
      new import_obsidian.Notice("Error finding backlinks");
  ...
  async createOrUpdateNote(title, content) {
    ...
    if (activeView && activeView.file) {
      ...
      if (activeFile.basename.toLowerCase() === base.toLowerCase() || activeFile.name.toLowerCase() === filename.toLowerCase()) {
    ...
    try {
      if (existingFile) {
        ...
        new import_obsidian.Notice(`Updated note: ${existingFile.path}`);
      } else {
        const filepath = folder ? `${folder}/${filename}` : filename;
        await this.app.vault.create(filepath, content);
        new import_obsidian.Notice(`Created new note: ${filepath}`);
      }
    } catch (error) {
      console.error("Error creating/updating note:", error);
      new import_obsidian.Notice("Error creating or updating note");
    }
  }
  onunload() {
    if (this.isRecording) {
...
var AIAssistantSettingTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian.Setting(containerEl).setName("OpenAI API Key").setDesc("Enter your OpenAI API key for AI features").addText((text) => text.setPlaceholder("sk-...").setValue(this.plugin.settings.openaiApiKey).onChange(async (value) => {
      this.plugin.settings.openaiApiKey = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian.Setting(containerEl).setName("Auto-tag after dictation").setDesc("Automatically generate tags after voice dictation").addToggle((toggle) => toggle.setValue(this.plugin.settings.autoTagEnabled).onChange(async (value) => {
      this.plugin.settings.autoTagEnabled = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian.Setting(containerEl).setName("Daily Notes Folder").setDesc("Folder to store generated daily notes and tasks").addText((text) => text.setPlaceholder("Daily Notes").setValue(this.plugin.settings.dailyNotesFolder).onChange(async (value) => {
      this.plugin.settings.dailyNotesFolder = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian.Setting(containerEl).setName("AI Chat Model").setDesc("OpenAI model to use for AI chat interactions").addDropdown((dropdown) => dropdown.addOption("gpt-4", "GPT-4 (Recommended)").addOption("gpt-3.5-turbo", "GPT-3.5 Turbo (Faster)").setValue(this.plugin.settings.chatModel).onChange(async (value) => {
